LAB.register({
  ...
  params: [
    ...
    { key: 'trail', label: 'Trail length', min: 200, max: 8000, step: 100, value: 7600 },
  ...
  draw(ctx, w, h, t, p, s, theme, a) {
    ...
    // slow 3D turn so the path keeps revealing new structure; only with sound (still in silence)
    var dtv = (a && isFinite(a.dt)) ? LABUTIL.clamp(a.dt, 0, 0.1) : 0.016;
    s.spin = (s.spin || 0) + dtv * act * 0.5;
    var ang = LABUTIL.clamp(p.projAngle, 0, 6.2832) + s.spin;
    var beta = s.beta;
    var DT = 0.008;
    var cap = s.MAXTRAIL;
    ...
    ctx.lineJoin = 'round';
    ctx.lineCap = 'round';
    // wide bright leading edge, tapering to a thin tail that fades to nothing with age
    var headW = LABUTIL.clamp(3.8 + react * (1.8 * lv + 1.4 * bt), 2.6, 8);
    ...
    for (var b = 0; b < BANDS; b++) {
      ...
      var frac = (sIdx + eIdx) * 0.5 / Math.max(1, nDraw - 1);   // 0 oldest .. 1 newest
      var aMid = frac * frac;                                     // tail disappears, head stays lit
      ctx.lineWidth = LABUTIL.lerp(0.6, headW, frac);
    ...
    if (isFinite(Ph[0]) && isFinite(Ph[1])) {
      ...
      ctx.beginPath();
      var headR = LABUTIL.clamp(3.4 + react * (2.5 * lv + 3.5 * bt), 2, 11);
